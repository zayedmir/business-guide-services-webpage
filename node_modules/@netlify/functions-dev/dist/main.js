...
import { basename as basename2, extname as extname2, isAbsolute, join as join2, resolve } from "path";
...
var DOS_DIRECTORY_ATTRIBUTE = 16;
var getEntryMode = (entry) => entry.externalFileAttributes >> 16 & 65535;
var isDirectoryEntry = (entry, mode) => {
...
var extractEntry = async (zipFile, entry, targetDirectory) => {
  ...
  const extractedMode = (mode || (isDirectory ? 493 : 420)) & 511;
  if (isDirectory) {
...
var extractZip = async (archivePath, targetDirectory) => {
...
var FunctionsRegistry = class {
  ...
  async registerFunction(name, func, isReload = false) {
    ...
    if (extname2(func.mainFile) === ".zip") {
      ...
      try {
        const v2EntryPointPath = join2(unzippedDirectory, "___netlify-entry-point.mjs");
        await stat(v2EntryPointPath);
        func.mainFile = v2EntryPointPath;
      } catch {
        func.mainFile = join2(unzippedDirectory, basename2(manifestEntry.mainFile));
      }
  ...
  async scan(relativeDirs) {
    const directories = relativeDirs.filter((dir) => Boolean(dir)).map((dir) => isAbsolute(dir) ? dir : join2(this.projectRoot, dir));
