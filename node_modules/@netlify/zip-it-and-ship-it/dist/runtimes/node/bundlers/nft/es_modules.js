    ...
    if (!isEntrypointESM({ basePath, esmPaths, mainFile })) {
        ...
        if (featureFlags.zisi_error_cjs_in_esm_scope && extension === MODULE_FILE_EXTENSION.JS) {
            ...
            if (scopePackageJson.type === 'module') {
                throw new FunctionBundlingUserError(`The function file '${mainFile}' is a CommonJS module, but the closest 'package.json' declares '"type": "module"', so Node.js will fail to load it at runtime. Either use ESM syntax ('import'/'export'), rename the file extension to '.cjs', or remove '"type": "module"' from the 'package.json'.`, { functionName: name, runtime: RUNTIME.JAVASCRIPT, bundler: NODE_BUNDLER.NFT });
            }
        }
        return {
