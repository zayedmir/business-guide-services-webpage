...
const processArchive = async ({ cache, path }) => {
    const stat = await cachedLstat(cache.lstatCache, path);
    return {
...
const zipFunction = async function ({ config, destFolder, filename, srcPath, isInternal }) {
...
const runtime = { findFunctionsInPaths, findFunctionInPath, name: RUNTIME.CONTAINER, zipFunction };
