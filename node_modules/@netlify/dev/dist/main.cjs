...
var import_server_dev = require("@netlify/server-dev");
...
var NetlifyDev = class {
  ...
  async handleInEphemeralDirectory(readRequest, getWriteRequest, destPath, options = {}) {
    ...
    if (redirectMatch) {
      ...
      if (imageMatch2) {
        const response2 = await imageMatch2.handle(getServerAddress());
  ...
  async start() {
    ...
    if (this.#features.server) {
      ...
      const serverHandler = new import_server_dev.ServerHandler({
        accountID: config?.siteInfo?.account_id,
        fileWatcher,
        geolocation,
        logger: this.#logger,
